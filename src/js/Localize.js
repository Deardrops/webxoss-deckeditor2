...
const Localize = (() => {
  ...
  Localize.DOM = function(namespace) {
    let localization = Localize.map.DOM[namespace]
    if (!localization) {
      console.warn('no such namespace: ' + namespace + ' .')
      return
    }
    for (let id in localization) {
      let el = document.getElementById(id)
      if (!el) {
        console.warn('no such element: ' + id + ' .')
        continue
      }
      let value = localization[id]
      if (isStr(value)) {
  ...
  Localize.noOptions = function(label) {
    ...
    let title = Localize.labelToTitle(label)
    let endPhase = [
      'END_ENER_PHASE',
      'END_GROW_PHASE',
      'END_MAIN_PHASE',
  ...
  Localize.desc = function(desc) {
    ...
    // let feedback = info.pid + type + idx
    if (!info) {
      return desc
    }
    let map = {
