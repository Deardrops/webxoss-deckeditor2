...
TextualRule.prototype.parse = function(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    let word = words[i]
    let keyword = this.parseWord(word)
    if (!keyword) continue
...
TextualRule.prototype.parseWord = function(word) {
  for (let keyword in this.map) {
    let matchWords = this.map[keyword]
...
let TypeRule = new TextualRule('cardType', {
...
let RarityRule = new TextualRule('rarity', {
  'c': ['c'],
  'r': ['r'],
  'lc': ['lc'],
  'sr': ['sr'],
...
SkillRule.parse = function(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    let word = words[i]
    let effectProp = this.parseWord(word)
  ...
  return function filter(info) {
    ...
    return effectProps.some(function(effectProp) {
      let effectTexts = info[effectProp]
...
NumericRule.prototype.parseWord = function(word) {
  for (let i = 0; i < this.keywords.length; i++) {
    let keyword = Localize.traditionalize(this.keywords[i])
...
let PowerRule = new NumericRule('power', ['力量', 'パワー', 'power', 'сила', '파워'])
let LevelRule = new NumericRule('level', ['等级', 'レベル', 'level', 'lv.', 'lv', 'уровень', 'livello', '레벨'])
let LimitRule = new NumericRule('limit', ['界限', 'リミット', 'limite', 'limit', 'ограничение', '리미트'])
...
//
///////////////////////////////////////////////////////////////
let NumberRule = new NumericRule('', [''])
NumberRule.parse = function(words) {
  ...
  return function filter(info) {
    return ranges.every(function(range) {
      ...
      let value = range.every(matchLevel) ? info.level : info.power
      let min = range[0]
      let max = range[1]
      return (value >= min) && (value <= max)
    })
...
IllustRule.parse = function(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    let word = words[i]
    let illust = this.parseWord(word)
    if (!illust) continue
    illusts.push(illust)
    words.splice(i, 1)
    i--
  ...
  return function filter(info) {
    ...
    return illusts.some(function(illust) {
      let cardIllust = info.illust.toLowerCase()
...
WxidRule.parseWord = function(word) {
  let match = word.match(/^(wx\d{2}|wd\d{2}|pr|sp\d{2})-?(re\d{0,2}|cb\d{0,2}|\d{0,3}[a|b]?)$/)
  if (!match) return null
...
WxbidRule.parseWord = function (word) {
  let match = word.match(/^(wxb)-?(\d{0,3}?)$/)
  if (!match) return null
  return match[1] + '-' + match[2]
}
...
NameRule.normalizeString = function(str) {
  let chars = str.replace(this.dotRegex, '').split('')
  chars.forEach(function(char, idx) {
    let i = this.fullWidth.indexOf(char)
  ...
  return function filter(info) {
    return wordSets.every(function(wordSet) {
      ...
      return wordSet.some(function(word) {
        let name = that.normalizeString(Localize.cardName(info))
        let w = that.normalizeString(word)
